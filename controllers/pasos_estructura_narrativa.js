...
exports.createPaso = async (req, res) => {
    try {
        ...
        res.status(201).json({ message: 'Paso de estructura narrativa creado correctamente', data: nuevoPaso });
    } catch (error) {
        res.status(500).json({ error: 'Error al crear el paso', detalle: error.message });
    }
};
...
exports.updatePaso = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Paso de estructura narrativa actualizado correctamente', data: pasoActualizado });
    } catch (error) {
